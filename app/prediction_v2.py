...
class TripPredictorV2:
    ...
    def prepare_features(self, distance, departure_time, origin_id):
        ...
        # Convert string stop_id to integer (e.g., 'BT01' -> 1)
        if isinstance(origin_id, str) and origin_id.startswith('BT'):
            origin_id = int(origin_id.replace('BT', ''))
    ...
    def generate_schedule(self, stops, current_time):
        ...
        for i, stop in enumerate(stops):
            ...
            # Convert stop_id to integer for prediction
            origin_id = int(stop['stop_id'].replace('BT', ''))
